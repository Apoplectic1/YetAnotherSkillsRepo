# docs-architecture-audit — delta spec

Scope note: this spec covers only the requirements this change establishes for the AUDIT skill
(`skills/docs-architecture-audit/SKILL.md`); it is not a retroactive full spec of the skill.
...
does not hold SHALL be corrected or dropped with a note — never presented as verified. This gate
lives once in AUDIT's machinery and is inherited by the skills that REQUIRED-reuse the fan-out.
