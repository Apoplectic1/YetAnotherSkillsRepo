...
## Purpose
TBD - created by archiving change publish-to-github. Update Purpose after archive.
## Requirements
...
The repo SHALL have an `origin` remote pointing at `github.com/Apoplectic1/docs-architecture`, and publication SHALL push only `main` (the distribution-ready ref); `dev` remains a local authoring branch.

#### Scenario: Publish
- **WHEN** publication runs
- **THEN** `git remote -v` shows `origin` → `Apoplectic1/docs-architecture`, `main` is pushed with upstream tracking, and no `dev` branch exists on the remote
